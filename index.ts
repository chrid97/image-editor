...
const blurValue = document.getElementById("blur-slider");
...
function addColor(currentPixelIndex, value, colorOffset) {
    newImage[currentPixelIndex + colorOffset] = clamp(
        newImage[currentPixelIndex + colorOffset] + value
    );
}

function setGrayscale(currentPixelIndex: number, imageData: number[]): void {
    const avg =
        newImage[currentPixelIndex] * 0.2126 +
        newImage[currentPixelIndex + 1] * 0.7152 +
        newImage[currentPixelIndex + 2] * 0.0722;
    newImage[currentPixelIndex] = avg; // red
    newImage[currentPixelIndex + 1] = avg; // green
    newImage[currentPixelIndex + 2] = avg; // blue
}

function invertColors(currentPixelIndex: number): void {
    newImage[currentPixelIndex] = clamp(255 - newImage[currentPixelIndex]); // red
    newImage[currentPixelIndex + 1] = clamp(
        255 - newImage[currentPixelIndex + 1]
    ); // green
    newImage[currentPixelIndex + 2] = clamp(
        255 - newImage[currentPixelIndex + 2]
    ); // blue
}
...
function changeBrightness(currentPixelIndex, brightness) {
    ...
    newImage[currentPixelIndex + 1] = clamp(
        newImage[currentPixelIndex + 1] + brightness
    );
    newImage[currentPixelIndex + 2] = clamp(
        newImage[currentPixelIndex + 2] + brightness
    );
}
...
function runPipeline() {
    ...
    for (let y = 0; y < srcImage.height; y++) {
        for (let x = 0; x < srcImage.width; x++) {
            let currentIndex = getPixelIndex(x, y);
    ...
    if (zoomDistance > 0 || zoomDistance < 0) {
        resizedImage = nearestNeighbor(
            newImage,
            new ImageData(
                srcImage.width + zoomDistance,
                srcImage.height + zoomDistance
            )
        );
    }
    ...
    if (zoomDistance > 0 || zoomDistance < 0) {
        ctx.putImageData(
            resizedImage,
            canvas.width / 2 - resizedImage.width / 2,
            canvas.height / 2 - resizedImage.height / 2
        );
    } else {
        ctx.putImageData(
            imgData,
            canvas.width / 2 - imgData.width / 2,
            canvas.height / 2 - imgData.height / 2
        );
    }
}
...
function flipHorizontally() {
    for (let x = 0; x < srcImage.width; x++) {
        for (let y = 0; y < srcImage.height / 2; y++) {
            ...
            const bottomIndex =
                getPixelIndex(srcImage.width - x, srcImage.height - y) - 1;
...
Array.from(sliderLabels).forEach((element) => {
    ...
    element.addEventListener("mouseover", (event) => {
        ...
        const value = Number(
            document.getElementById(`${lowerCaseString}-slider`).value
        );
        if (value > 0 || value < 0) {
...
document
    ...
    .addEventListener("wheel", ({ wheelDeltaY }) => {
        if (wheelDeltaY > 0) {
            zoomDistance += srcImage.width * 0.1;
            runPipeline();
...
function nearestNeighbor(src, dst) {
    ...
    for (let y = 0; y < dst.height; y++) {
        for (let x = 0; x < dst.width; x++) {
            const srcX = Math.floor((x * imgData.width) / dst.width);
            const srcY = Math.floor((y * imgData.height) / dst.height);

            let srcPos = (srcY * imgData.width + srcX) * 4;

            dst.data[pos++] = src[srcPos++]; // R
            dst.data[pos++] = src[srcPos++]; // G
            dst.data[pos++] = src[srcPos++]; // B
            dst.data[pos++] = src[srcPos++]; // A
        }
    }
    return dst;
}
